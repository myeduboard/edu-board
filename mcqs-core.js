...
function aiExplanationFormatInstruction(pName) {
    return 'EXPLANATION FORMAT (critical — ALWAYS use this structure; this OVERRIDES and REPLACES any pre-existing explanation\'s formatting): '
        ...
        + 'FIRST decide which of these two branches applies, then follow ONLY that branch\'s section list — the two branches are NOT combined: '
        + 'BRANCH A — NUMERICAL/MATHEMATICAL/QUANTITATIVE questions (needs a calculation, formula, equation, or step-wise derivation — e.g. a maths/physics/chemistry numerical problem): use ONLY these two sections, in order — (a1) a short <h3>Key Concept</h3> section (1-2 <p> sentences naming/recalling the concept, formula, or law this question depends on); (a2) an <h3>Step-by-Step Solution</h3> heading with one <h4>Step 1: ...</h4>, <h4>Step 2: ...</h4>, etc. minor-heading per step (each with its own <p> working directly beneath it, plus a <ul>/<table> only if it genuinely clarifies that step), ending with a closing <h4>Final Answer</h4> heading whose <p> states the resulting value. Do NOT add an "Incorrect Options Analysis" or "Important Points" section for this branch — the step-by-step working and final answer are the complete explanation. '
        + 'BRANCH B — EVERY OTHER kind of question — general knowledge, general studies, biology, history, geography, civics/polity, current affairs, or a purely conceptual/theoretical/definitional maths, physics, or chemistry question with NO calculation involved: do NOT use a "Key Concept" heading or step headings at all. Use these three sections, in order — (b1) an <h3>Correct Answer Explanation</h3> heading with <p> paragraphs (and a <ul>/<table> only if genuinely useful) that state the correct answer\'s substance and justify it conceptually; (b2) an <h3>Incorrect Options Analysis</h3> section: for each of the OTHER (incorrect) options, one short <p> (or one <li> inside a single <ul>, one item per option) that names that option\'s own content/value directly and explains concretely why it is wrong or does not fit the question — refer to it ONLY by its actual text/value, never by its letter/label; (b3) an <h3>Important Points</h3> section with genuinely useful, closely-related facts for quick revision, formatted as a <ul>/<ol> or — where the facts are naturally parallel/comparable — a <table>. For example, for a question on the highest peak of a state, list the highest peaks of the other major states/regions in a table (columns like State | Peak | Height); for a formula-based conceptual question, list related formulas/units/constants; for a historical-date question, list a short related timeline of nearby events; for a classification question, list the other members of that classification. Keep this section genuinely informative and exam-useful, not filler. '
...
function aiStepsInstruction(pName) {
    return `STEP-BY-STEP MATH SOLUTIONS (when applicable): if — and ONLY if — the question is numerical/mathematical/quantitative (requires a calculation, formula, equation, or step-wise numerical/logical derivation), you MUST use BRANCH A from the EXPLANATION FORMAT above (Key Concept + Step-by-Step Solution ONLY): one <h4>Step N: ...</h4> minor-heading per step (translate "Step" into ${pName} if ${pName} is not English) with that step's own <p> beneath it, never skipping a calculation, substitution, or logical link, ending with a closing <h4>Final Answer</h4> heading and <p> stating the resulting value/answer. Keep all math in LaTeX ($...$). Do NOT add "Incorrect Options Analysis" or "Important Points" sections after the steps — Branch A explanations end at "Final Answer". For purely conceptual/factual/definitional questions with no calculation involved, do NOT force artificial steps or a "Key Concept" heading — use Branch B (Correct Answer Explanation + Incorrect Options Analysis + Important Points) instead.`;
}
